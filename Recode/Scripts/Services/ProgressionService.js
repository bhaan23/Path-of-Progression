...
export default class ProgressionService {
	...
	checkTopNodes(trigger) {
		...
		for (let i = this.nodeService.topNodes.length -1; i > -1; i--) {
			const nodeId = this.nodeService.topNodeIds[i];
			if (trigger === this.nodeService.nodeMap[nodeId].progressionData.completionTrigger.toLowerCase()) {
	...
	checkNodesForItems() {
		...
		for (let i = this.nodeService.topNodes.length -1; i > -1; i--) {
			const nodeId = this.nodeService.topNodeIds[i];
			if (hasNodeTriggeredFromItem(this.nodeService.nodeMap[nodeId], this.characterInventoryService)) {
				this.completeNode(nodeId);
			}
		}
	}
	...
	completeNode(nodeId) {
		...
		this.nodeService.topNodeIds.splice(this.nodeService.topNodeIds, 1);
		
		// Add dependants to the top nodes
		this.nodeService.topNodeIds.concat(this.nodeService.nodeMap[nodeId].dependantNodeIds);
